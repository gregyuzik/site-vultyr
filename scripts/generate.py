...
STRINGS = {
    "en": {
        ...
        "home_features": [
            ...
            ("bell-ringing-regular.svg", "Smart Alerts",
             "Down and recovery notifications with each service's favicon attached on iOS. Mute known incidents, star critical services. The Mac app drives the alerts \u2014 see Cross-Device Sync below."),
            ...
            ("cloud-check-regular.svg", "Mac Is the Hub \u2014 Required for Reliable Alerts",
             "iOS limits background polling, so iPhone alone can miss or delay alerts. Keep the Mac app running and it polls every 60 seconds, broadcasting status changes to iPhone, iPad, Watch, and Vision Pro via iCloud. The Mac app is the recommended way to use Vultyr."),
        ...
        "support_faqs": [
            ...
            ("How do Cross-Device Alerts work?",
             "The Mac app is the hub. Keep it running (menu bar or full window) and it polls every 60 seconds. When a status change is detected, it writes a lightweight signal to iCloud Key-Value Store; your iPhone, iPad, Watch, Apple TV, and Vision Pro pick up the change and run their own local check. No keys, no tokens, no setup \u2014 just enable \"Cross-Device Alerts\" in settings on any device. Without a Mac acting as the hub, alerts rely on iOS background execution and will be delayed or missed.",
             "<p>The Mac app is the hub. Keep it running (menu bar or full window) and it polls every 60 seconds. When a status change is detected, it writes a lightweight signal to iCloud Key-Value Store; your iPhone, iPad, Watch, Apple TV, and Vision Pro pick up the change and run their own local check. No keys, no tokens, no setup \u2014 just enable \"Cross-Device Alerts\" in settings on any device. Without a Mac acting as the hub, alerts rely on iOS background execution and will be delayed or missed.</p>"),
            ("Do I need the Mac app for reliable alerts?",
             "Yes \u2014 we strongly recommend it. iOS limits background execution, so iPhone and iPad can only check every 5-15 minutes (configurable) and may defer further on low battery, Low Power Mode, or poor connectivity. The Mac app polls continuously (every 60 seconds when plugged in) and broadcasts status changes to your other devices via iCloud. Without a Mac running Vultyr, iOS, watchOS, and tvOS alerts still work but can be significantly delayed or missed. For real-time monitoring, keep the Mac app running \u2014 it's tiny in the menu bar and is how Vultyr is meant to be used.",
             "<p>Yes \u2014 we strongly recommend it. iOS limits background execution, so iPhone and iPad can only check every 5-15 minutes (configurable) and may defer further on low battery, Low Power Mode, or poor connectivity. The Mac app polls continuously (every 60 seconds when plugged in) and broadcasts status changes to your other devices via iCloud. Without a Mac running Vultyr, iOS, watchOS, and tvOS alerts still work but can be significantly delayed or missed. For real-time monitoring, keep the Mac app running \u2014 it's tiny in the menu bar and is how Vultyr is meant to be used.</p>"),
    ...
    "ru": {
        ...
        "home_features": [
            ...
            ("bell-ringing-regular.svg",
             "Умные уведомления",
             "Оповещения о падениях и восстановлениях \u2014 на iOS с фавиконом каждого сервиса. Отключайте известные инциденты, отмечайте важные сервисы. Приложение Mac обеспечивает оповещения \u2014 см. ниже."),
            ...
            ("cloud-check-regular.svg",
             "Mac \u2014 центр уведомлений (нужен для надёжных оповещений)",
             "iOS ограничивает фоновый опрос, поэтому один iPhone может пропускать или задерживать оповещения. Держите приложение Mac запущенным \u2014 оно опрашивает каждые 60 секунд и рассылает изменения статуса на iPhone, iPad, Watch и Vision Pro через iCloud. Приложение Mac \u2014 рекомендуемый способ использования Vultyr."),
            ("devices-regular.svg",
             "Все платформы Apple",
    ...
    "home_features_sub": "Ingen app-konti, ingen servere, ingen sporing i appen. Bare status.",
    ...
    "home_features": [
        ("chart-bar-regular.svg", "Live-statusdashboard",
         "AWS, GitHub, Cloudflare, Slack, Stripe, Discord, OpenAI, Anthropic og 200+ flere \u2014 alt samlet ét sted."),
        ("bell-ringing-regular.svg", "Smarte underretninger",
         "Underretninger om nedbrud og genoprettelse med hver tjenestes favicon vedhæftet på iOS. Sæt kendte hændelser på lydløs, stjernemarkér kritiske tjenester. Mac-appen driver underretningerne \u2014 se Synkronisering på tværs af enheder nedenfor."),
        ...
        ("cloud-check-regular.svg", "Mac er centralen \u2014 kræves for pålidelige underretninger",
         "iOS begrænser baggrundsafstemning, så iPhone alene kan gå glip af eller forsinke underretninger. Hold Mac-appen kørende, og den afstemmer hvert 60. sekund og sender statusændringer til iPhone, iPad, Watch og Vision Pro via iCloud. Mac-appen er den anbefalede måde at bruge Vultyr på."),
        ...
        ("battery-charging-regular.svg", "Batterivenlig afstemning",
         "Smart automatisk opdatering tilpasser sig batteri, strømtilstand og temperatur. Hvert minut på Mac, 5-15 på iPhone."),
    ...
    "support_faqs": [
        ...
        ("Har jeg brug for Mac-appen for at få pålidelige underretninger?",
         "Ja \u2014 vi anbefaler det kraftigt. iOS begrænser baggrundsafvikling, så iPhone og iPad kan kun tjekke hvert 5.-15. minut (kan indstilles) og kan udskyde yderligere ved lavt batteri, Low Power Mode eller dårlig forbindelse. Mac-appen afstemmer løbende (hvert 60. sekund, når enheden er tilsluttet strøm) og sender statusændringer til dine andre enheder via iCloud. Uden en Mac, der kører Vultyr, fungerer underretninger på iOS, watchOS og tvOS stadig, men de kan være væsentligt forsinkede eller gå tabt. For realtidsovervågning bør du holde Mac-appen kørende \u2014 den er minimal i menubjælken, og det er sådan, Vultyr er tænkt til at blive brugt.",
        ...
        ("Indsamler vultyr-appen mine data?",
         "Nej. Appen har ingen konti, ingen servere, ingen sporing i appen, ingen analytik i appen. Alle dine overvågede tjenester forbliver på din enhed. Bemærk: dette website (vultyr.app) bruger cookieløs Google Analytics til aggregerede besøgstal \u2014 se privatlivspolitikken for detaljer.",
         "<p>Nej. Appen har ingen konti, ingen servere, ingen sporing i appen, ingen analytik i appen. Alle dine overvågede tjenester forbliver på din enhed. Bemærk: dette website (vultyr.app) bruger cookieløs Google Analytics til aggregerede besøgstal \u2014 se <a href=\"/privacy.html\">privatlivspolitikken</a> for detaljer.</p>"),
    ...
    "home_features_sub": "Keine App-Konten, keine Server, kein In-App-Tracking. Nur Status.",
    ...
    "home_features": [
        ("chart-bar-regular.svg",
         ...
         "AWS, GitHub, Cloudflare, Slack, Stripe, Discord, OpenAI, Anthropic und über 200 weitere \u2014 alles an einem Ort."),
        ("bell-ringing-regular.svg",
         "Intelligente Benachrichtigungen",
         "Ausfall- und Wiederherstellungsmeldungen \u2014 auf iOS mit dem Favicon jedes Dienstes. Bekannte Vorfälle stummschalten, kritische Dienste markieren. Die Mac-App steuert die Benachrichtigungen \u2014 siehe \u201EGeräteübergreifende Synchronisation\u201C unten."),
        ("microphone-regular.svg",
         "Siri und Shortcuts",
        ...
        ("cloud-check-regular.svg",
         "Mac ist die Zentrale \u2014 nötig für zuverlässige Benachrichtigungen",
         "iOS begrenzt das Polling im Hintergrund, daher kann ein iPhone allein Benachrichtigungen verpassen oder verzögern. Lass die Mac-App laufen \u2014 sie fragt alle 60 Sekunden ab und sendet Statusänderungen per iCloud an iPhone, iPad, Watch und Vision Pro. Die Mac-App ist der empfohlene Weg, Vultyr zu nutzen."),
        ("devices-regular.svg",
        ...
        ("battery-charging-regular.svg",
         "Akku-bewusstes Polling",
         "Intelligentes Auto-Refresh passt sich an Akku, Stromzustand und Temperatur an. Jede Minute auf dem Mac, 5 bis 15 Minuten auf dem iPhone."),
        ("palette-regular.svg", f"{THEMES_COUNT} Themes",
    ...
    "support_faqs": [
        ...
        ("Erfasst die Vultyr-App meine Daten?",
         "Nein. Die App hat keine Konten, keine Server, kein In-App-Tracking, keine In-App-Analytik. Alle deine beobachteten Dienste bleiben auf deinem Gerät. Hinweis: Diese Website (vultyr.app) nutzt cookieloses Google Analytics für aggregierte Besucherzahlen \u2014 Details in der Datenschutzerklärung.",
         "<p>Nein. Die App hat keine Konten, keine Server, kein In-App-Tracking, keine In-App-Analytik. Alle deine beobachteten Dienste bleiben auf deinem Gerät. Hinweis: Diese Website (vultyr.app) nutzt cookieloses Google Analytics für aggregierte Besucherzahlen \u2014 Details in der <a href=\"/privacy.html\">Datenschutzerklärung</a>.</p>"),
    ...
    "home_features_sub": "Sin cuentas de app, sin servidores, sin rastreo dentro de la app. Solo estado.",
    ...
    "home_features": [
        ("chart-bar-regular.svg", "Panel de estado en vivo",
         "AWS, GitHub, Cloudflare, Slack, Stripe, Discord, OpenAI, Anthropic y más de 200 más \u2014 todo en un solo lugar."),
        ("bell-ringing-regular.svg", "Alertas inteligentes",
         "Notificaciones de caídas y recuperación con el favicon de cada servicio adjunto en iOS. Silencia incidentes conocidos, marca como favoritos los servicios críticos. La app de Mac impulsa las alertas \u2014 consulta Sincronización entre dispositivos más abajo."),
        ...
        ("cloud-check-regular.svg", "El Mac es el centro \u2014 necesario para alertas fiables",
         "iOS limita el sondeo en segundo plano, por lo que el iPhone por sí solo puede omitir o retrasar alertas. Mantén la app de Mac en ejecución y sondeará cada 60 segundos, difundiendo los cambios de estado a iPhone, iPad, Watch y Vision Pro mediante iCloud. La app de Mac es la forma recomendada de usar Vultyr."),
        ...
        ("battery-charging-regular.svg", "Sondeo consciente de la batería",
         "La actualización automática inteligente se adapta a la batería, el estado de alimentación y la temperatura. Cada minuto en Mac, entre 5 y 15 en iPhone."),
    ...
    "home_features_sub": "Aucun compte dans l\u2019app, aucun serveur, aucun suivi int\u00e9gr\u00e9. Juste l\u2019\u00e9tat.",
    ...
    "home_features": [
        ("chart-bar-regular.svg", "Tableau de bord d\u2019\u00e9tat en direct",
         "AWS, GitHub, Cloudflare, Slack, Stripe, Discord, OpenAI, Anthropic et plus de 200 autres \u2014 tous au m\u00eame endroit."),
        ("bell-ringing-regular.svg", "Alertes intelligentes",
         "Notifications de panne et de r\u00e9tablissement avec le favicon de chaque service sur iOS. Mettez en sourdine les incidents connus, \u00e9pinglez les services critiques. L\u2019app Mac pilote les alertes \u2014 voir Synchronisation entre appareils ci-dessous."),
        ...
        ("cloud-check-regular.svg", "Le Mac est le hub \u2014 requis pour des alertes fiables",
         "iOS limite l\u2019interrogation en arri\u00e8re-plan, donc l\u2019iPhone seul peut manquer ou retarder des alertes. Gardez l\u2019app Mac en fonctionnement et elle interroge toutes les 60\u00a0secondes, diffusant les changements d\u2019\u00e9tat vers l\u2019iPhone, l\u2019iPad, la Watch et le Vision Pro via iCloud. L\u2019app Mac est la fa\u00e7on recommand\u00e9e d\u2019utiliser Vultyr."),
        ...
        ("battery-charging-regular.svg", "Interrogation adapt\u00e9e \u00e0 la batterie",
         "L\u2019actualisation automatique s\u2019adapte \u00e0 la batterie, \u00e0 l\u2019alimentation et \u00e0 la temp\u00e9rature. Chaque minute sur Mac, toutes les 5\u201315\u00a0minutes sur iPhone."),
    ...
    "support_faqs": [
        ...
        ("Comment fonctionnent les alertes entre appareils\u00a0?",
         "L\u2019app Mac est le hub. Gardez-la en fonctionnement (barre de menus ou fen\u00eatre pleine) et elle interroge toutes les 60\u00a0secondes. Lorsqu\u2019un changement d\u2019\u00e9tat est d\u00e9tect\u00e9, elle \u00e9crit un signal l\u00e9ger dans l\u2019iCloud Key-Value Store\u00a0; vos iPhone, iPad, Watch, Apple TV et Vision Pro captent le changement et ex\u00e9cutent leur propre v\u00e9rification locale. Aucune cl\u00e9, aucun jeton, aucune configuration \u2014 activez simplement \u00ab\u00a0Alertes entre appareils\u00a0\u00bb dans les r\u00e9glages sur n\u2019importe quel appareil. Sans un Mac jouant le r\u00f4le de hub, les alertes reposent sur l\u2019ex\u00e9cution en arri\u00e8re-plan d\u2019iOS et seront retard\u00e9es ou manqu\u00e9es.",
         "<p>L\u2019app Mac est le hub. Gardez-la en fonctionnement (barre de menus ou fen\u00eatre pleine) et elle interroge toutes les 60\u00a0secondes. Lorsqu\u2019un changement d\u2019\u00e9tat est d\u00e9tect\u00e9, elle \u00e9crit un signal l\u00e9ger dans l\u2019iCloud Key-Value Store\u00a0; vos iPhone, iPad, Watch, Apple TV et Vision Pro captent le changement et ex\u00e9cutent leur propre v\u00e9rification locale. Aucune cl\u00e9, aucun jeton, aucune configuration \u2014 activez simplement \u00ab\u00a0Alertes entre appareils\u00a0\u00bb dans les r\u00e9glages sur n\u2019importe quel appareil. Sans un Mac jouant le r\u00f4le de hub, les alertes reposent sur l\u2019ex\u00e9cution en arri\u00e8re-plan d\u2019iOS et seront retard\u00e9es ou manqu\u00e9es.</p>"),
        ...
        ("L\u2019application vultyr collecte-t-elle mes donn\u00e9es\u00a0?",
         "Non. L\u2019application n\u2019a aucun compte, aucun serveur, aucun suivi int\u00e9gr\u00e9, aucune analyse int\u00e9gr\u00e9e. Tous vos services surveill\u00e9s restent sur votre appareil. Remarque\u00a0: ce site (vultyr.app) utilise Google Analytics sans cookies pour compter les visiteurs agr\u00e9g\u00e9s \u2014 voir la Politique de confidentialit\u00e9 pour les d\u00e9tails.",
         "<p>Non. L\u2019application n\u2019a aucun compte, aucun serveur, aucun suivi int\u00e9gr\u00e9, aucune analyse int\u00e9gr\u00e9e. Tous vos services surveill\u00e9s restent sur votre appareil. Remarque\u00a0: ce site (vultyr.app) utilise Google Analytics sans cookies pour compter les visiteurs agr\u00e9g\u00e9s \u2014 voir la <a href=\"/fr/privacy.html\">Politique de confidentialit\u00e9</a> pour les d\u00e9tails.</p>"),
    ...
    "home_features_sub": "Nessun account, nessun server, nessun tracciamento interno. Solo stato.",
    ...
    "home_features": [
        ("chart-bar-regular.svg", "Dashboard di stato in tempo reale",
         "AWS, GitHub, Cloudflare, Slack, Stripe, Discord, OpenAI, Anthropic e oltre 200 altri \u2014 tutti in un unico posto."),
        ("bell-ringing-regular.svg", "Avvisi intelligenti",
         "Notifiche di disservizio e ripristino con la favicon di ogni servizio allegata su iOS. Silenzia gli incidenti noti, metti tra i preferiti i servizi critici. L'app Mac gestisce gli avvisi \u2014 vedi Sincronizzazione tra dispositivi qui sotto."),
        ...
        ("cloud-check-regular.svg", "Il Mac \u00e8 il fulcro \u2014 indispensabile per avvisi affidabili",
         "iOS limita il polling in background, quindi l'iPhone da solo pu\u00f2 perdere o ritardare gli avvisi. Tieni l'app Mac in esecuzione: esegue il polling ogni 60 secondi e trasmette i cambi di stato a iPhone, iPad, Watch e Vision Pro tramite iCloud. L'app Mac \u00e8 il modo consigliato per usare Vultyr."),
        ...
        ("battery-charging-regular.svg", "Polling che rispetta la batteria",
         "L'aggiornamento automatico si adatta a batteria, alimentazione e temperatura. Ogni minuto su Mac, 5\u201315 minuti su iPhone."),
    ...
    "support_faqs": [
        ...
        ("Come funzionano gli avvisi tra dispositivi?",
         "L'app Mac \u00e8 il fulcro. Tienila in esecuzione (nella barra dei menu o a finestra piena) ed eseguir\u00e0 il polling ogni 60 secondi. Quando viene rilevato un cambio di stato, scrive un segnale leggero in iCloud Key-Value Store; il tuo iPhone, iPad, Watch, Apple TV e Vision Pro rilevano il cambiamento ed eseguono il proprio controllo locale. Nessuna chiave, nessun token, nessuna configurazione \u2014 basta attivare \u00abAvvisi tra dispositivi\u00bb nelle impostazioni di qualsiasi dispositivo. Senza un Mac che funge da fulcro, gli avvisi dipendono dall'esecuzione in background di iOS e saranno ritardati o persi.",
         "<p>L'app Mac \u00e8 il fulcro. Tienila in esecuzione (nella barra dei menu o a finestra piena) ed eseguir\u00e0 il polling ogni 60 secondi. Quando viene rilevato un cambio di stato, scrive un segnale leggero in iCloud Key-Value Store; il tuo iPhone, iPad, Watch, Apple TV e Vision Pro rilevano il cambiamento ed eseguono il proprio controllo locale. Nessuna chiave, nessun token, nessuna configurazione \u2014 basta attivare \u00abAvvisi tra dispositivi\u00bb nelle impostazioni di qualsiasi dispositivo. Senza un Mac che funge da fulcro, gli avvisi dipendono dall'esecuzione in background di iOS e saranno ritardati o persi.</p>"),
        ...
        ("L'app vultyr raccoglie i miei dati?",
         "No. L'app non ha account, non ha server, non ha tracciamento interno, non ha analytics interni. Tutti i servizi che monitori restano sul tuo dispositivo. Nota: questo sito (vultyr.app) utilizza Google Analytics senza cookie per i conteggi aggregati dei visitatori \u2014 consulta l'Informativa sulla privacy per i dettagli.",
         "<p>No. L'app non ha account, non ha server, non ha tracciamento interno, non ha analytics interni. Tutti i servizi che monitori restano sul tuo dispositivo. Nota: questo sito (vultyr.app) utilizza Google Analytics senza cookie per i conteggi aggregati dei visitatori \u2014 consulta l'<a href=\"/it/privacy.html\">Informativa sulla privacy</a> per i dettagli.</p>"),
    ...
    "home_features_sub": "アプリアカウント不要、サーバー不要、アプリ内トラッキングなし。ただステータスを。",
    ...
    "home_features": [
        ("chart-bar-regular.svg", "リアルタイムステータスダッシュボード",
         "AWS、GitHub、Cloudflare、Slack、Stripe、Discord、OpenAI、Anthropicなど200以上 — すべてを一ヵ所で。"),
        ("bell-ringing-regular.svg", "スマートアラート",
         "障害と復旧の通知、iOSでは各サービスのファビコン付き。既知のインシデントはミュート、重要なサービスはスター付けできます。Macアプリがアラートを駆動します — 下のクロスデバイス同期をご覧ください。"),
        ...
        ("cloud-check-regular.svg", "Macがハブ — 確実な通知のために必須",
         "iOSはバックグラウンドポーリングを制限するため、iPhone単体では通知を取り逃したり遅延したりします。Macアプリを起動しておけば60秒ごとにポーリングし、iCloud経由でiPhone、iPad、Watch、Vision Proにステータス変化をブロードキャストします。MacアプリはVultyrの推奨される使い方です。"),
        ...
        ("battery-charging-regular.svg", "バッテリーを考慮したポーリング",
         "スマートな自動更新がバッテリー、電源状態、温度に合わせて調整されます。Macでは毎分、iPhoneでは5〜15分おき。"),
    ...
    "support_faqs": [
        ...
        ("クロスデバイスアラートはどのように動作しますか?",
         "Macアプリがハブです。メニューバーまたはフルウィンドウで起動したままにしておけば、60秒ごとにポーリングします。ステータスの変化を検出すると、軽量なシグナルをiCloud Key-Value Storeに書き込み、iPhone、iPad、Watch、Apple TV、Vision Proがその変化を受け取ってそれぞれローカルでチェックを実行します。鍵もトークンもセットアップも不要 — どのデバイスの設定でも「クロスデバイスアラート」を有効にするだけです。Macをハブにしない場合、アラートはiOSのバックグラウンド実行に依存するため遅延または欠落します。",
         "<p>Macアプリがハブです。メニューバーまたはフルウィンドウで起動したままにしておけば、60秒ごとにポーリングします。ステータスの変化を検出すると、軽量なシグナルをiCloud Key-Value Storeに書き込み、iPhone、iPad、Watch、Apple TV、Vision Proがその変化を受け取ってそれぞれローカルでチェックを実行します。鍵もトークンもセットアップも不要 — どのデバイスの設定でも「クロスデバイスアラート」を有効にするだけです。Macをハブにしない場合、アラートはiOSのバックグラウンド実行に依存するため遅延または欠落します。</p>"),
        ("確実なアラートのためにMacアプリは必要ですか?",
         "はい — 強くお勧めします。iOSはバックグラウンド実行を制限するため、iPhoneとiPadは5〜15分ごと(設定可能)にしかチェックできず、バッテリー残量の低下、低電力モード、通信状況の悪化でさらに遅延することがあります。Macアプリは連続的にポーリングし(電源接続時は60秒ごと)、iCloud経由で他のデバイスにステータスの変化をブロードキャストします。VultyrをMacで動かしていない場合、iOS、watchOS、tvOSのアラートは動作しますが大幅に遅延したり欠落したりする可能性があります。リアルタイム監視のためにはMacアプリを起動したままにしておいてください — メニューバーに小さく常駐し、これがVultyr本来の使い方です。",
        ...
        ("vultyrアプリは私のデータを収集しますか?",
         "いいえ。アプリにはアカウントもサーバーもアプリ内トラッキングもアプリ内分析もありません。監視中のサービスはすべてお使いのデバイスに留まります。注意: このウェブサイト(vultyr.app)は集計訪問者数のためにCookieを使用しないGoogle Analyticsを利用しています — 詳細はプライバシーポリシーをご覧ください。",
         "<p>いいえ。アプリにはアカウントもサーバーもアプリ内トラッキングもアプリ内分析もありません。監視中のサービスはすべてお使いのデバイスに留まります。注意: このウェブサイト(vultyr.app)は集計訪問者数のためにCookieを使用しないGoogle Analyticsを利用しています — 詳細は<a href=\"/privacy.html\">プライバシーポリシー</a>をご覧ください。</p>"),
    ...
    "home_features_sub": "앱 계정도, 서버도, 인앱 추적도 없습니다. 오직 상태만.",
    ...
    "home_features": [
        ("chart-bar-regular.svg",
         "실시간 상태 대시보드",
         "AWS, GitHub, Cloudflare, Slack, Stripe, Discord, OpenAI, Anthropic 및 200개 이상 — 모두 한곳에서."),
        ("bell-ringing-regular.svg",
         "스마트 알림",
         "장애 및 복구 알림을 iOS에서는 각 서비스의 파비콘과 함께 제공합니다. 알고 있는 장애는 음소거하고 중요한 서비스는 별표로 표시하세요. Mac 앱이 알림을 구동합니다 — 아래 기기 간 동기화를 참조하세요."),
        ("microphone-regular.svg",
         "Siri 및 Shortcuts",
        ...
        ("cloud-check-regular.svg",
         "Mac이 허브 — 안정적인 알림에 필수",
         "iOS는 백그라운드 폴링을 제한하므로 iPhone만으로는 알림이 누락되거나 지연될 수 있습니다. Mac 앱을 실행한 상태로 유지하면 60초마다 폴링하고 iCloud를 통해 iPhone, iPad, Watch, Vision Pro에 상태 변경을 방송합니다. Mac 앱은 Vultyr를 사용하는 권장 방식입니다."),
        ("devices-regular.svg",
         "모든 Apple 플랫폼",
        ...
        ("battery-charging-regular.svg",
         "배터리 인식 폴링",
         "스마트 자동 새로 고침이 배터리, 전원 상태, 발열에 적응합니다. Mac에서는 1분마다, iPhone에서는 5~15분마다."),
    ...
    "support_faqs": [
        ("vultyr는 서비스 상태를 얼마나 자주 점검하나요?",
         "Mac에서는 전원 연결 시 60초마다 점검합니다. iPhone에서는 5, 10 또는 15분마다(설정 가능)이며, 조건이 허용될 때 주기적인 백그라운드 점검도 수행됩니다. Apple Watch에서는 15분마다, Apple TV에서는 5분마다 점검합니다. 폴링은 배터리 수준, 전원 상태, 발열 조건에 따라 자동으로 적응합니다.",
         "<p>Mac에서는 전원 연결 시 60초마다 점검합니다. iPhone에서는 5, 10 또는 15분마다(설정 가능)이며, 조건이 허용될 때 주기적인 백그라운드 점검도 수행됩니다. Apple Watch에서는 15분마다, Apple TV에서는 5분마다 점검합니다. 폴링은 배터리 수준, 전원 상태, 발열 조건에 따라 자동으로 적응합니다.</p>"),
        ("기기 간 알림은 어떻게 작동하나요?",
         "Mac 앱이 허브입니다. Mac 앱을 실행 상태로 유지(메뉴 막대 또는 전체 창)하면 60초마다 폴링합니다. 상태 변경이 감지되면 iCloud Key-Value Store에 경량 신호를 기록하며, iPhone, iPad, Watch, Apple TV, Vision Pro가 변경을 감지하고 자체적인 로컬 점검을 실행합니다. 키도, 토큰도, 설정도 필요 없습니다 — 어느 기기에서나 설정에서 「기기 간 알림」을 켜기만 하면 됩니다. Mac이 허브 역할을 하지 않으면 알림은 iOS 백그라운드 실행에 의존하며 지연되거나 누락됩니다.",
         "<p>Mac 앱이 허브입니다. Mac 앱을 실행 상태로 유지(메뉴 막대 또는 전체 창)하면 60초마다 폴링합니다. 상태 변경이 감지되면 iCloud Key-Value Store에 경량 신호를 기록하며, iPhone, iPad, Watch, Apple TV, Vision Pro가 변경을 감지하고 자체적인 로컬 점검을 실행합니다. 키도, 토큰도, 설정도 필요 없습니다 — 어느 기기에서나 설정에서 「기기 간 알림」을 켜기만 하면 됩니다. Mac이 허브 역할을 하지 않으면 알림은 iOS 백그라운드 실행에 의존하며 지연되거나 누락됩니다.</p>"),
        ("안정적인 알림에 Mac 앱이 필요한가요?",
         "네 — 강력히 권장합니다. iOS는 백그라운드 실행을 제한하므로 iPhone과 iPad는 5~15분마다(설정 가능)만 점검할 수 있으며, 낮은 배터리, 저전력 모드 또는 연결 상태가 나쁠 때 추가로 지연될 수 있습니다. Mac 앱은 지속적으로 폴링하며(전원 연결 시 60초마다) iCloud를 통해 다른 기기에 상태 변경을 방송합니다. Vultyr를 실행하는 Mac이 없으면 iOS, watchOS, tvOS 알림은 여전히 작동하지만 상당히 지연되거나 누락될 수 있습니다. 실시간 모니터링을 위해 Mac 앱을 실행 상태로 유지하세요 — 메뉴 막대에서 매우 작으며 이것이 Vultyr가 사용되도록 의도된 방식입니다.",
         "<p>네 — 강력히 권장합니다. iOS는 백그라운드 실행을 제한하므로 iPhone과 iPad는 5~15분마다(설정 가능)만 점검할 수 있으며, 낮은 배터리, 저전력 모드 또는 연결 상태가 나쁠 때 추가로 지연될 수 있습니다. Mac 앱은 지속적으로 폴링하며(전원 연결 시 60초마다) iCloud를 통해 다른 기기에 상태 변경을 방송합니다. Vultyr를 실행하는 Mac이 없으면 iOS, watchOS, tvOS 알림은 여전히 작동하지만 상당히 지연되거나 누락될 수 있습니다. 실시간 모니터링을 위해 Mac 앱을 실행 상태로 유지하세요 — 메뉴 막대에서 매우 작으며 이것이 Vultyr가 사용되도록 의도된 방식입니다.</p>"),
        ...
        ("vultyr 앱은 제 데이터를 수집하나요?",
         "아니요. 앱에는 계정도, 서버도, 인앱 추적도, 인앱 분석도 없습니다. 모니터링 중인 모든 서비스는 사용자 기기에 남아있습니다. 참고: 본 웹사이트(vultyr.app)는 집계 방문자 수를 위해 쿠키 없는 Google Analytics를 사용합니다 — 자세한 내용은 개인정보 처리방침을 참조하세요.",
         "<p>아니요. 앱에는 계정도, 서버도, 인앱 추적도, 인앱 분석도 없습니다. 모니터링 중인 모든 서비스는 사용자 기기에 남아있습니다. 참고: 본 웹사이트(vultyr.app)는 집계 방문자 수를 위해 쿠키 없는 Google Analytics를 사용합니다 — 자세한 내용은 <a href=\"/ko/privacy.html\">개인정보 처리방침</a>을 참조하세요.</p>"),
    ...
    "home_features_sub": "Ingen appkontoer, ingen servere, ingen sporing i appen. Bare status.",
    ...
    "home_features": [
        ("chart-bar-regular.svg",
         ...
         "AWS, GitHub, Cloudflare, Slack, Stripe, Discord, OpenAI, Anthropic og 200+ flere — alt samlet på ett sted."),
        ("bell-ringing-regular.svg",
         "Smarte varsler",
         "Varsler om nedetid og gjenoppretting med hver tjenestes favikon vedlagt på iOS. Demp kjente hendelser, stjernemerk kritiske tjenester. Mac-appen driver varslene — se «Synkronisering mellom enheter» nedenfor."),
        ("microphone-regular.svg",
         "Siri og Shortcuts",
        ...
        ("cloud-check-regular.svg",
         "Mac er navet — nødvendig for pålitelige varsler",
         "iOS begrenser bakgrunnsopprop, så iPhone alene kan gå glipp av eller forsinke varsler. Hold Mac-appen i gang, så opprører den hvert 60. sekund og kringkaster statusendringer til iPhone, iPad, Watch og Vision Pro via iCloud. Mac-appen er den anbefalte måten å bruke Vultyr på."),
        ("devices-regular.svg",
        ...
        ("battery-charging-regular.svg",
         "Batteribevisst opprop",
         "Smart automatisk oppdatering tilpasser seg batteri, strømtilstand og temperatur. Hvert minutt på Mac, 5–15 på iPhone."),
        ("palette-regular.svg", f"{THEMES_COUNT} temaer",
    ...
    "support_faqs": [
        ...
        ("Samler vultyr-appen inn dataene mine?",
         "Nei. Appen har ingen kontoer, ingen servere, ingen sporing i appen, ingen analyse i appen. Alle tjenestene du følger, blir værende på enheten din. Merk: dette nettstedet (vultyr.app) bruker Google Analytics uten informasjonskapsler for aggregerte besøkstellinger — se personvernerklæringen for detaljer.",
         "<p>Nei. Appen har ingen kontoer, ingen servere, ingen sporing i appen, ingen analyse i appen. Alle tjenestene du følger, blir værende på enheten din. Merk: dette nettstedet (vultyr.app) bruker Google Analytics uten informasjonskapsler for aggregerte besøkstellinger — se <a href=\"/nb/privacy.html\">personvernerklæringen</a> for detaljer.</p>"),
    ...
    "home_features_sub": "Geen accounts, geen servers, geen tracking in de app. Alleen status.",
    ...
    "home_features": [
        ("chart-bar-regular.svg", "Live statusdashboard",
         "AWS, GitHub, Cloudflare, Slack, Stripe, Discord, OpenAI, Anthropic en 200+ andere \u2014 alles op \u00e9\u00e9n plek."),
        ("bell-ringing-regular.svg", "Slimme meldingen",
         "Meldingen bij storingen en herstel, met op iOS het favicon van elke service erbij. Dempt bekende incidenten, markeer kritieke services. De Mac\u2011app stuurt de meldingen aan \u2014 zie Synchronisatie tussen apparaten hieronder."),
        ...
        ("cloud-check-regular.svg", "Mac is het centrum \u2014 nodig voor betrouwbare meldingen",
         "iOS beperkt pollen op de achtergrond, dus een iPhone alleen kan meldingen missen of vertragen. Houd de Mac\u2011app draaiend en die peilt elke 60 seconden, en verspreidt statuswijzigingen naar iPhone, iPad, Watch en Vision Pro via iCloud. De Mac\u2011app is de aanbevolen manier om Vultyr te gebruiken."),
        ...
        ("battery-charging-regular.svg", "Batterijbewust pollen",
         "Slim automatisch vernieuwen past zich aan batterij, stroomvoorziening en temperatuur aan. Elke minuut op Mac, 5\u201315 minuten op iPhone."),
    ...
    "support_faqs": [
        ("Hoe vaak controleert Vultyr de servicestatus?",
         "Op Mac: elke 60 seconden bij een stroomaansluiting. Op iPhone: elke 5, 10 of 15 minuten (instelbaar), met periodieke achtergrondcontroles waar dat kan. Op Apple Watch: elke 15 minuten. Op Apple TV: elke 5 minuten. Het pollen past zich automatisch aan batterijniveau, stroomvoorziening en temperatuur aan.",
         "<p>Op Mac: elke 60 seconden bij een stroomaansluiting. Op iPhone: elke 5, 10 of 15 minuten (instelbaar), met periodieke achtergrondcontroles waar dat kan. Op Apple Watch: elke 15 minuten. Op Apple TV: elke 5 minuten. Het pollen past zich automatisch aan batterijniveau, stroomvoorziening en temperatuur aan.</p>"),
        ("Hoe werken meldingen tussen apparaten?",
         "De Mac\u2011app is het centrum. Houd die draaiend (in de menubalk of als volledig venster) en die peilt elke 60 seconden. Zodra een statuswijziging wordt gedetecteerd, schrijft de app een klein signaal naar de iCloud Key\u2011Value Store; je iPhone, iPad, Watch, Apple TV en Vision Pro pikken de wijziging op en voeren hun eigen lokale controle uit. Geen sleutels, geen tokens, geen setup \u2014 zet gewoon \u201eMeldingen tussen apparaten\u201d aan in de instellingen op een willekeurig apparaat. Zonder Mac als centrum leunen meldingen op iOS\u2011achtergronduitvoering en zullen ze vertraagd of gemist worden.",
         "<p>De Mac\u2011app is het centrum. Houd die draaiend (in de menubalk of als volledig venster) en die peilt elke 60 seconden. Zodra een statuswijziging wordt gedetecteerd, schrijft de app een klein signaal naar de iCloud Key\u2011Value Store; je iPhone, iPad, Watch, Apple TV en Vision Pro pikken de wijziging op en voeren hun eigen lokale controle uit. Geen sleutels, geen tokens, geen setup \u2014 zet gewoon \u201eMeldingen tussen apparaten\u201d aan in de instellingen op een willekeurig apparaat. Zonder Mac als centrum leunen meldingen op iOS\u2011achtergronduitvoering en zullen ze vertraagd of gemist worden.</p>"),
        ("Heb ik de Mac\u2011app nodig voor betrouwbare meldingen?",
         "Ja \u2014 we raden het sterk aan. iOS beperkt achtergronduitvoering, dus iPhone en iPad kunnen alleen elke 5\u201315 minuten controleren (instelbaar) en kunnen verder uitstellen bij een lage batterij, Energiebesparingsmodus of slechte verbinding. De Mac\u2011app peilt continu (elke 60 seconden bij een stroomaansluiting) en verspreidt statuswijzigingen naar je andere apparaten via iCloud. Zonder een Mac waarop Vultyr draait, werken meldingen op iOS, watchOS en tvOS nog steeds, maar kunnen ze aanzienlijk vertraagd of gemist worden. Voor realtime monitoring laat je de Mac\u2011app draaien \u2014 hij is klein in de menubalk en zo is Vultyr bedoeld om gebruikt te worden.",
         "<p>Ja \u2014 we raden het sterk aan. iOS beperkt achtergronduitvoering, dus iPhone en iPad kunnen alleen elke 5\u201315 minuten controleren (instelbaar) en kunnen verder uitstellen bij een lage batterij, Energiebesparingsmodus of slechte verbinding. De Mac\u2011app peilt continu (elke 60 seconden bij een stroomaansluiting) en verspreidt statuswijzigingen naar je andere apparaten via iCloud. Zonder een Mac waarop Vultyr draait, werken meldingen op iOS, watchOS en tvOS nog steeds, maar kunnen ze aanzienlijk vertraagd of gemist worden. Voor realtime monitoring laat je de Mac\u2011app draaien \u2014 hij is klein in de menubalk en zo is Vultyr bedoeld om gebruikt te worden.</p>"),
        ...
        ("Verzamelt de Vultyr\u2011app mijn gegevens?",
         "Nee. De app heeft geen accounts, geen servers, geen tracking in de app en geen analytics in de app. Al je gevolgde services blijven op je apparaat. Let op: deze website (vultyr.app) gebruikt cookieloze Google Analytics voor geaggregeerde bezoekersaantallen \u2014 zie het Privacybeleid voor details.",
         "<p>Nee. De app heeft geen accounts, geen servers, geen tracking in de app en geen analytics in de app. Al je gevolgde services blijven op je apparaat. Let op: deze website (vultyr.app) gebruikt cookieloze Google Analytics voor geaggregeerde bezoekersaantallen \u2014 zie het <a href=\"/privacy.html\">Privacybeleid</a> voor details.</p>"),
    ...
    "home_features_sub": "Sem contas, sem servidores, sem rastreamento no app. Apenas status.",
    ...
    "home_features": [
        ("chart-bar-regular.svg",
         ...
         "AWS, GitHub, Cloudflare, Slack, Stripe, Discord, OpenAI, Anthropic e mais de 200 outros — tudo em um só lugar."),
        ("bell-ringing-regular.svg",
         "Alertas inteligentes",
         "Notificações de quedas e recuperações com o favicon de cada serviço anexado no iOS. Silencie incidentes conhecidos, destaque serviços críticos. O app para Mac é quem dispara os alertas — veja Sincronização entre dispositivos abaixo."),
        ("microphone-regular.svg",
         "Siri e Shortcuts",
        ...
        ("cloud-check-regular.svg",
         "O Mac é o hub — necessário para alertas confiáveis",
         "O iOS limita a execução em segundo plano, então o iPhone sozinho pode perder ou atrasar alertas. Mantenha o app para Mac em execução e ele faz verificações a cada 60 segundos, transmitindo mudanças de status para iPhone, iPad, Watch e Vision Pro via iCloud. O app para Mac é a forma recomendada de usar o Vultyr."),
        ("devices-regular.svg",
        ...
        ("battery-charging-regular.svg",
         "Verificação consciente da bateria",
         "A atualização automática se adapta à bateria, ao estado de energia e à temperatura. A cada minuto no Mac, 5 a 15 no iPhone."),
        ("palette-regular.svg", f"{THEMES_COUNT} temas",
    ...
    "support_faqs": [
        ...
        ("O app vultyr coleta meus dados?",
         "Não. O app não tem contas, servidores, rastreamento interno nem análise interna. Todos os serviços que você acompanha ficam no seu dispositivo. Observação: este site (vultyr.app) usa Google Analytics sem cookies para contagens agregadas de visitantes — veja a Política de privacidade para detalhes.",
         "<p>Não. O app não tem contas, servidores, rastreamento interno nem análise interna. Todos os serviços que você acompanha ficam no seu dispositivo. Observação: este site (vultyr.app) usa Google Analytics sem cookies para contagens agregadas de visitantes — veja a <a href=\"/pt-BR/privacy.html\">Política de privacidade</a> para detalhes.</p>"),
    ...
    "home_features_sub": "Inga appkonton, inga servrar, ingen spårning i appen. Bara status.",
    ...
    "home_features": [
        ("chart-bar-regular.svg", "Live-statusinstrumentpanel",
         "AWS, GitHub, Cloudflare, Slack, Stripe, Discord, OpenAI, Anthropic och 200+ till — allt på ett ställe."),
        ("bell-ringing-regular.svg", "Smarta aviseringar",
         "Aviseringar om nedtid och återställning med varje tjänsts favikon bifogad på iOS. Tysta kända incidenter, stjärnmärk kritiska tjänster. Mac-appen driver aviseringarna — se Synkronisering mellan enheter nedan."),
        ...
        ("cloud-check-regular.svg", "Mac är navet — krävs för tillförlitliga aviseringar",
         "iOS begränsar bakgrundsavsökning, så enbart iPhone kan missa eller fördröja aviseringar. Håll Mac-appen igång så avsöker den var 60:e sekund och skickar statusändringar till iPhone, iPad, Watch och Vision Pro via iCloud. Mac-appen är det rekommenderade sättet att använda Vultyr."),
        ...
        ("battery-charging-regular.svg", "Batterismart avsökning",
         "Smart autoavsökning anpassas efter batteri, strömförsörjning och värmeförhållanden. Varje minut på Mac, 5–15 på iPhone."),
    ...
    "support_faqs": [
        ...
        ("Samlar vultyr-appen in mina data?",
         "Nej. Appen har inga konton, inga servrar, ingen spårning i appen och ingen analys i appen. Alla dina bevakade tjänster stannar på din enhet. Observera: denna webbplats (vultyr.app) använder cookielös Google Analytics för aggregerade besöksräkningar — se integritetspolicyn för detaljer.",
         "<p>Nej. Appen har inga konton, inga servrar, ingen spårning i appen och ingen analys i appen. Alla dina bevakade tjänster stannar på din enhet. Observera: denna webbplats (vultyr.app) använder cookielös Google Analytics för aggregerade besöksräkningar — se <a href=\"/sv/privacy.html\">integritetspolicyn</a> för detaljer.</p>"),
    ...
    "home_features_sub": "Uygulama hesabı yok, sunucu yok, uygulama içi takip yok. Yalnızca durum.",
    ...
    "home_features": [
        ("chart-bar-regular.svg", "Canlı durum kontrol paneli",
         "AWS, GitHub, Cloudflare, Slack, Stripe, Discord, OpenAI, Anthropic ve 200+ daha fazlası \u2014 hepsi tek bir yerde."),
        ("bell-ringing-regular.svg", "Akıllı uyarılar",
         "iOS'ta her servisin favicon'u eklenmiş kesinti ve kurtarma bildirimleri. Bilinen olayları sessize alın, kritik servisleri yıldızlayın. Uyarıları Mac uygulaması sürer \u2014 aşağıdaki Cihazlar Arası Senkronizasyon bölümüne bakın."),
        ...
        ("cloud-check-regular.svg", "Mac merkezdir \u2014 Güvenilir uyarılar için gerekli",
         "iOS arka plan yoklamasını kısıtlar, bu yüzden tek başına iPhone uyarıları kaçırabilir veya geciktirebilir. Mac uygulamasını çalışır durumda tutun; her 60 saniyede bir yoklama yaparak durum değişikliklerini iCloud üzerinden iPhone, iPad, Watch ve Vision Pro'ya yayınlar. Mac uygulaması Vultyr'ı kullanmanın önerilen yoludur."),
        ...
        ("battery-charging-regular.svg", "Pile duyarlı yoklama",
         "Akıllı otomatik yenileme; pil, güç durumu ve termal koşullara uyum sağlar. Mac'te her dakika, iPhone'da 5\u201315 dakikada bir."),
    ...
    "support_faqs": [
        ...
        ("Cihazlar Arası Uyarılar nasıl çalışır?",
         "Merkez Mac uygulamasıdır. Çalışır durumda tutun (menü çubuğunda veya tam pencerede) ve her 60 saniyede bir yoklama yapar. Bir durum değişikliği algılandığında, iCloud Anahtar-Değer Deposu'na hafif bir sinyal yazar; iPhone'unuz, iPad'iniz, Watch'ınız, Apple TV'niz ve Vision Pro'nuz değişikliği alır ve kendi yerel kontrolünü çalıştırır. Anahtar yok, jeton yok, kurulum yok \u2014 herhangi bir cihazdaki ayarlardan \u201cCihazlar Arası Uyarılar\u201d seçeneğini etkinleştirmeniz yeter. Merkez görevi gören bir Mac olmadan, uyarılar iOS arka plan çalışmasına bağlı kalır ve gecikir ya da kaçırılır.",
         "<p>Merkez Mac uygulamasıdır. Çalışır durumda tutun (menü çubuğunda veya tam pencerede) ve her 60 saniyede bir yoklama yapar. Bir durum değişikliği algılandığında, iCloud Anahtar-Değer Deposu'na hafif bir sinyal yazar; iPhone'unuz, iPad'iniz, Watch'ınız, Apple TV'niz ve Vision Pro'nuz değişikliği alır ve kendi yerel kontrolünü çalıştırır. Anahtar yok, jeton yok, kurulum yok \u2014 herhangi bir cihazdaki ayarlardan \u201cCihazlar Arası Uyarılar\u201d seçeneğini etkinleştirmeniz yeter. Merkez görevi gören bir Mac olmadan, uyarılar iOS arka plan çalışmasına bağlı kalır ve gecikir ya da kaçırılır.</p>"),
        ("Güvenilir uyarılar için Mac uygulamasına ihtiyacım var mı?",
         "Evet \u2014 kesinlikle öneriyoruz. iOS arka plan çalışmasını kısıtlar, bu nedenle iPhone ve iPad yalnızca 5\u201315 dakikada bir kontrol edebilir (yapılandırılabilir) ve düşük pil, Düşük Güç Modu veya zayıf bağlantıda daha fazla geciktirebilir. Mac uygulaması sürekli yoklama yapar (şarja bağlıyken 60 saniyede bir) ve durum değişikliklerini iCloud üzerinden diğer cihazlarınıza yayınlar. Vultyr çalıştıran bir Mac olmadan iOS, watchOS ve tvOS uyarıları çalışmaya devam eder ama önemli ölçüde gecikebilir veya kaçırılabilir. Gerçek zamanlı izleme için Mac uygulamasını çalışır durumda tutun \u2014 menü çubuğunda çok az yer kaplar ve Vultyr'ın bu şekilde kullanılması amaçlanmıştır.",
        ...
        ("Vultyr uygulaması verilerimi topluyor mu?",
         "Hayır. Uygulamanın hesabı, sunucusu, uygulama içi takibi veya uygulama içi analitiği yoktur. İzlediğiniz tüm servisler cihazınızda kalır. Not: Bu web sitesi (vultyr.app) toplu ziyaretçi sayıları için çerezsiz Google Analytics kullanır \u2014 ayrıntılar için Gizlilik Politikası'na bakın.",
         "<p>Hayır. Uygulamanın hesabı, sunucusu, uygulama içi takibi veya uygulama içi analitiği yoktur. İzlediğiniz tüm servisler cihazınızda kalır. Not: Bu web sitesi (vultyr.app) toplu ziyaretçi sayıları için çerezsiz Google Analytics kullanır \u2014 ayrıntılar için <a href=\"/privacy.html\">Gizlilik Politikası</a>'na bakın.</p>"),
    ...
    "home_features_sub": "Không tài khoản ứng dụng, không máy chủ, không theo dõi trong ứng dụng. Chỉ có trạng thái.",
    ...
    "home_features": [
        ("chart-bar-regular.svg", "Bảng điều khiển trạng thái trực tiếp",
         "AWS, GitHub, Cloudflare, Slack, Stripe, Discord, OpenAI, Anthropic và hơn 200 dịch vụ khác \u2014 tất cả ở một nơi."),
        ("bell-ringing-regular.svg", "Cảnh báo thông minh",
         "Thông báo khi mất kết nối và khôi phục, kèm favicon của từng dịch vụ trên iOS. Tắt tiếng sự cố đã biết, gắn sao các dịch vụ quan trọng. Ứng dụng Mac là nguồn cảnh báo \u2014 xem Đồng bộ giữa các thiết bị bên dưới."),
        ...
        ("cloud-check-regular.svg", "Mac là trung tâm \u2014 Cần thiết để cảnh báo đáng tin cậy",
         "iOS giới hạn việc truy vấn nền, nên nếu chỉ có iPhone thì cảnh báo có thể bị bỏ lỡ hoặc trễ. Hãy giữ ứng dụng Mac chạy và nó sẽ truy vấn mỗi 60 giây, phát các thay đổi trạng thái đến iPhone, iPad, Watch và Vision Pro qua iCloud. Ứng dụng Mac là cách khuyến nghị để sử dụng Vultyr."),
        ...
        ("battery-charging-regular.svg", "Truy vấn theo pin",
         "Tự động làm mới thích ứng với pin, nguồn điện và nhiệt độ máy. Mỗi phút trên Mac, 5\u201315 phút trên iPhone."),
    ...
    "support_faqs": [
        ...
        ("Cảnh báo giữa các thiết bị hoạt động như thế nào?",
         "Ứng dụng Mac là trung tâm. Hãy giữ nó chạy (trong thanh menu hoặc cửa sổ đầy đủ) và nó sẽ truy vấn mỗi 60 giây. Khi phát hiện thay đổi trạng thái, nó ghi một tín hiệu nhẹ vào iCloud Key-Value Store; iPhone, iPad, Watch, Apple TV và Vision Pro của bạn sẽ nhận thay đổi và tự thực hiện kiểm tra cục bộ. Không cần khóa, không token, không phải thiết lập \u2014 chỉ cần bật \u201cCảnh báo giữa các thiết bị\u201d trong cài đặt trên bất kỳ thiết bị nào. Không có Mac đóng vai trò trung tâm, cảnh báo sẽ phụ thuộc vào thực thi nền của iOS và sẽ bị trễ hoặc bỏ lỡ.",
         "<p>Ứng dụng Mac là trung tâm. Hãy giữ nó chạy (trong thanh menu hoặc cửa sổ đầy đủ) và nó sẽ truy vấn mỗi 60 giây. Khi phát hiện thay đổi trạng thái, nó ghi một tín hiệu nhẹ vào iCloud Key-Value Store; iPhone, iPad, Watch, Apple TV và Vision Pro của bạn sẽ nhận thay đổi và tự thực hiện kiểm tra cục bộ. Không cần khóa, không token, không phải thiết lập \u2014 chỉ cần bật \u201cCảnh báo giữa các thiết bị\u201d trong cài đặt trên bất kỳ thiết bị nào. Không có Mac đóng vai trò trung tâm, cảnh báo sẽ phụ thuộc vào thực thi nền của iOS và sẽ bị trễ hoặc bỏ lỡ.</p>"),
        ...
        ("Ứng dụng vultyr có thu thập dữ liệu của tôi không?",
         "Không. Ứng dụng không có tài khoản, không máy chủ, không theo dõi trong ứng dụng, không phân tích trong ứng dụng. Tất cả các dịch vụ bạn theo dõi vẫn ở trên thiết bị của bạn. Lưu ý: trang web này (vultyr.app) sử dụng Google Analytics không dùng cookie để đếm lượt khách truy cập tổng hợp \u2014 xem Chính sách quyền riêng tư để biết chi tiết.",
         "<p>Không. Ứng dụng không có tài khoản, không máy chủ, không theo dõi trong ứng dụng, không phân tích trong ứng dụng. Tất cả các dịch vụ bạn theo dõi vẫn ở trên thiết bị của bạn. Lưu ý: trang web này (vultyr.app) sử dụng Google Analytics không dùng cookie để đếm lượt khách truy cập tổng hợp \u2014 xem <a href=\"/privacy.html\">Chính sách quyền riêng tư</a> để biết chi tiết.</p>"),
    ...
    "home_features_sub": "无需应用账号,无需服务器,无内置追踪。只有状态。",
    ...
    "home_features": [
        ("chart-bar-regular.svg", "实时状态仪表板",
         "AWS、GitHub、Cloudflare、Slack、Stripe、Discord、OpenAI、Anthropic 及 200+ 其他服务 —— 全部集中在一处。"),
        ("bell-ringing-regular.svg", "智能提醒",
         "在 iOS 上推送故障与恢复通知,并附带各服务图标。可静音已知事件、星标关键服务。Mac 应用负责驱动提醒 —— 请参阅下方的「跨设备同步」。"),
        ...
        ("cloud-check-regular.svg", "Mac 是枢纽 —— 可靠提醒的必要条件",
         "iOS 限制后台轮询,因此仅靠 iPhone 可能会漏掉或延迟提醒。让 Mac 应用保持运行,它会每 60 秒轮询一次,并通过 iCloud 将状态变化广播给 iPhone、iPad、Watch 和 Vision Pro。Mac 应用是使用 Vultyr 的推荐方式。"),
        ...
        ("battery-charging-regular.svg", "感知电量的轮询",
         "智能自动刷新会根据电量、供电状态和温度进行调整。Mac 上每分钟一次,iPhone 上 5–15 分钟一次。"),
    ...
    "support_faqs": [
        ...
        ("跨设备提醒是如何工作的?",
         "Mac 应用是枢纽。让它保持运行(菜单栏或完整窗口),它会每 60 秒轮询一次。一旦检测到状态变化,它会向 iCloud Key-Value Store 写入一个轻量信号;您的 iPhone、iPad、Watch、Apple TV 和 Vision Pro 会接收到该变化并各自执行本地检查。无需密钥、令牌或配置 —— 只需在任一设备的设置中启用「跨设备提醒」。如果没有 Mac 作为枢纽,提醒将依赖 iOS 的后台执行,可能被延迟或遗漏。",
         "<p>Mac 应用是枢纽。让它保持运行(菜单栏或完整窗口),它会每 60 秒轮询一次。一旦检测到状态变化,它会向 iCloud Key-Value Store 写入一个轻量信号;您的 iPhone、iPad、Watch、Apple TV 和 Vision Pro 会接收到该变化并各自执行本地检查。无需密钥、令牌或配置 —— 只需在任一设备的设置中启用「跨设备提醒」。如果没有 Mac 作为枢纽,提醒将依赖 iOS 的后台执行,可能被延迟或遗漏。</p>"),
        ...
        ("Vultyr 应用会收集我的数据吗?",
         "不会。应用没有账号、没有服务器、没有内置追踪,也没有内置分析。您关注的所有服务仅保存在您的设备上。注意:本网站(vultyr.app)使用无 Cookie 的 Google Analytics 统计聚合访问量 —— 详情请参阅隐私政策。",
         "<p>不会。应用没有账号、没有服务器、没有内置追踪,也没有内置分析。您关注的所有服务仅保存在您的设备上。注意:本网站(vultyr.app)使用无 Cookie 的 Google Analytics 统计聚合访问量 —— 详情请参阅<a href=\"/privacy.html\">隐私政策</a>。</p>"),
        ("如何在多台设备间同步我的服务?",
    ...
    "home_features_sub": "不需應用程式帳號、不需伺服器、無應用程式內追蹤。只有狀態。",
    ...
    "home_features": [
        ("chart-bar-regular.svg", "即時狀態儀表板",
         "AWS、GitHub、Cloudflare、Slack、Stripe、Discord、OpenAI、Anthropic 等 200+ 項服務 — 全部集中於一處。"),
        ("bell-ringing-regular.svg", "智慧通知",
         "中斷與恢復通知在 iOS 上會附加每項服務的 favicon。可靜音已知事件、星號標示關鍵服務。Mac 應用程式負責推送通知 — 請參閱下方的跨裝置同步。"),
        ...
        ("cloud-check-regular.svg", "Mac 是中樞 — 可靠通知的必要條件",
         "iOS 會限制背景輪詢,因此僅使用 iPhone 可能會錯過或延遲通知。讓 Mac 應用程式持續執行,它會每 60 秒輪詢一次,並透過 iCloud 將狀態變更廣播至 iPhone、iPad、Watch 與 Vision Pro。Mac 應用程式是使用 Vultyr 的建議方式。"),
        ...
        ("battery-charging-regular.svg", "電量感知輪詢",
         "智慧自動重新整理會依電量、電源狀態與機身溫度自動調整。Mac 上每分鐘一次,iPhone 上為 5 至 15 分鐘。"),
    ...
    "support_faqs": [
        ...
        ("跨裝置通知如何運作?",
         "Mac 應用程式是中樞。讓它持續執行(選單列或完整視窗皆可),它會每 60 秒輪詢一次。偵測到狀態變更時,它會寫入一個輕量訊號至 iCloud Key-Value Store;您的 iPhone、iPad、Watch、Apple TV 與 Vision Pro 會接收此變更並各自執行本機檢查。無需金鑰、無需權杖、無需設定 — 只要在任一裝置的設定中啟用「跨裝置通知」即可。若沒有 Mac 擔任中樞,通知將仰賴 iOS 背景執行,會被延遲或錯過。",
         "<p>Mac 應用程式是中樞。讓它持續執行(選單列或完整視窗皆可),它會每 60 秒輪詢一次。偵測到狀態變更時,它會寫入一個輕量訊號至 iCloud Key-Value Store;您的 iPhone、iPad、Watch、Apple TV 與 Vision Pro 會接收此變更並各自執行本機檢查。無需金鑰、無需權杖、無需設定 — 只要在任一裝置的設定中啟用「跨裝置通知」即可。若沒有 Mac 擔任中樞,通知將仰賴 iOS 背景執行,會被延遲或錯過。</p>"),
        ...
        ("vultyr 應用程式會收集我的資料嗎?",
         "不會。應用程式沒有帳號、沒有伺服器、沒有應用程式內追蹤、沒有應用程式內分析。您所關注的服務全部留在您的裝置上。請注意:本網站(vultyr.app)使用無 Cookie 的 Google Analytics 以彙總訪客計數 — 詳情請參閱隱私權政策。",
         "<p>不會。應用程式沒有帳號、沒有伺服器、沒有應用程式內追蹤、沒有應用程式內分析。您所關注的服務全部留在您的裝置上。請注意:本網站(vultyr.app)使用無 Cookie 的 Google Analytics 以彙總訪客計數 — 詳情請參閱<a href=\"/privacy.html\">隱私權政策</a>。</p>"),
        ("如何在各裝置間同步我的服務?",
